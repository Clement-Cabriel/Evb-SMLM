...
polarity_detection='positive'   # polarity used for the PSF detection. ='positive', 'negative', 'absolute' (i.e. sum of all events regardless of their polarity) or 'both' (i.e. positive count as +1 and negative count as -1)
...
smoothing_frame=1.              # Sigma value of the Gaussian filtering of the frame (display only)
number_frames_display=0       # Number of frames to display. Used for both the frames display
...
def Save_parameters_init(events,start_time):
    ...
    "polarity_detection":polarity_detection,"smoothing_frame":smoothing_frame,"threshold_detection":threshold_detection,"exclusion_radius":exclusion_radius,"min_diameter":min_diameter,"max_diameter":max_diameter,
...
def generate_single_frame(events,times):
    ...
    if polarity_detection=='positive':msk*=(events['p']==1)
    elif polarity_detection=='negative':msk*=(events['p']==0)
    sub_events=events[msk]
    for k in np.arange(len(sub_events)):
        xycoords=[int(np.floor(sub_events['y'][k])),int(np.floor(sub_events['x'][k]))]
        if polarity_detection=='positive' or polarity_detection=='negative' or polarity_detection=='absolute':
            frame[xycoords[0],xycoords[1]]+=1
        elif polarity_detection=='both':
            frame[xycoords[0],xycoords[1]]+=2.*(sub_events['p'][k]-0.5)
    if polarity_detection=='both':frame[frame<0]=0
    return frame
...
def display_frame_PSFs(frame,frame_number):    
    ...
    # frame=scipy.ndimage.gaussian_filter(frame,smoothing_frame)
    # sbp.imshow(frame,cmap='hot',interpolation='none')
    # # plt.plot(ROIs[:,1],ROIs[:,0],'xb',ms=10,mew=2.)
    # plt.plot(ROIs[:,1],ROIs[:,0],'sb',ms=10,mew=2.,markerfacecolor='none')
